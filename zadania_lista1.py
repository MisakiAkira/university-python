...
translate("to jest fajne")

#zadanie 12
def ile_znakow(text):
    slownik = {}
    for i in text:
        if not slownik.__contains__(i):
            slownik[i]=1
        else:
            slownik[i] += 1
    return slownik

print(ile_znakow("abbabcbdbabdbdbabababcbcbab"))

#zadanie 13
class PodelPrzezSzedem:
    def __init__(self, n):
        self.n = n
    def inter(self):
        return [i for i in range(1, self.n + 1) if i % 7 == 0]
    

for i in PodelPrzezSzedem(70).inter():
    print(i)
